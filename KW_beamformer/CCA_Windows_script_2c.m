% % Modified Kaiser Window beamformer for Concentric Circular Array (CCA)
% % Using Gradient Descent Algorithm 
% % Comparing KW vs. KW-Fixed-2 vs. Method-1 vs. Method-2
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% clear all ; clc ; close all ;
% 
% design_name = {'CCA (KW)', 'Method-1', 'Method-2'} ;
% 
...
% %design = load( 'CCA_design_half' ) ;
% %design = load( 'CCA_design_quarter' ) ;
% %design = load( 'CCA_design_one-eighth' ) ;
% 
% r_p = design.r_p ;
...
%     else
%         init_beta_prime{p} = 0.5*ones(1,length(f)) ; % initialize to 0 (all sensors active initially)
%     end
% end
% 
% init_w = cell(1,P) ;
...
% 
%     if strcmp(design_name{idx_design_name}, 'CCA (KW)')
% 
...
% %save(['CCA_half_varying_beamformer'], 'Delta_b_theta', 'Delta_b_phi', 'D', 'W', 'B', 'theta_range', 'phi_range' ) ;
% %save(['CCA_quarter_varying_beamformer'], 'Delta_b_theta', 'Delta_b_phi', 'D', 'W', 'B', 'theta_range', 'phi_range' ) ;
% %save(['CCA_one-eighth_varying_beamformer'], 'Delta_b_theta', 'Delta_b_phi', 'D', 'W', 'B', 'theta_range', 'phi_range' ) ;
% 
% exit ;
% 
% return ;
...
%loadname = 'CCA_half_varying_beamformer' ;
%loadname = 'CCA_quarter_varying_beamformer' ;
%loadname = 'CCA_one-eighth_varying_beamformer' ;
...
ylim([ 0, 22]) ; 
yticks( round( linspace(min(ylim), max(ylim), 5), 1 ) ) ;
hleg = legend({'KW'; 'Method-1'; 'Method-2'}); 
title(hleg, 'CCA (Nyquist)~$ \theta_{\mathrm{d}} = \phi_{\mathrm{d}} = 45^o$ ~$\theta_{\mathrm{BW}} = \phi_{\mathrm{BW}} = 40^o$~Beamformer','Interpreter','Latex');
b=gca;
set (b, 'XTick', fig_ticks); set (b, 'XTickLabel', fig_labels );
set(b,'FontName','Times New Roman','FontWeight','Bold','LineWidth',0.5,'FontSize',16);
a=findobj(gcf); % get the handles associated with the current figure
